/* Statistik: Wochenraster + Tages-Detail + Routen-Karte v16 */
(function () {
  'use strict';
  ...
  function showDayDetail(dateISO) {
    ...
    /* Klick-Handler für Übungs-Icons und Route (onclick statt addEventListener verhindert Listener-Akkumulation) */
  ...
  function showRouteMap(coords) {
    ...
    setTimeout(function () {
      ...
      routeMapLayers.forEach(function (l) { routeMap.removeLayer(l); });
      routeMapLayers = [];
      var line = L.polyline(coords, { color: '#042615', weight: 4, opacity: .85 }).addTo(routeMap);
      routeMap.fitBounds(line.getBounds(), { padding: [24, 24] });
      var startM = L.circleMarker(coords[0], { radius: 7, color: '#fff', fillColor: '#93C3D9', fillOpacity: 1, weight: 2 }).addTo(routeMap);
      var endM   = L.circleMarker(coords[coords.length - 1], { radius: 7, color: '#fff', fillColor: '#A6BD7B', fillOpacity: 1, weight: 2 }).addTo(routeMap);
      routeMapLayers.push(line, startM, endM);
    }, 60);
  }
