...
(function () {
  ...
  function showExercise(ex) {
    ...
    var wielandSrc = ex.world === 5
      ...
      : ex.world === 4
        ? 'assets/Wieland%20Schaetze.apng'
        : ex.world === 3
          ? 'assets/Wieland%20Timer.apng'
          : ex.world === 1
            ? 'assets/Wieland%20Spuernase.apng'
    ...
    layer.innerHTML =
      ...
      (ex.type === 'photo' ? '<div class="ex-photo-row"><p class="counter__hint" id="photo-count">0 / ' + (ex.target || 1) + '</p><button class="btn btn--brett btn-sm" id="photo-add" type="button">Foto aufnehmen</button></div>' : '') +
      '<div class="ex-bottom">' +
  ...
  function startGuidedRoute(start, key, attempt) {
    ...
    fetchRoute(start, Math.round(WW.distanceKm(checkin.energy) * 1000), key)
      ...
      .catch(function (err) {
        ...
        $('instruction-text').textContent = 'Route konnte nicht geladen werden.' + detail;
        var retryBtn = document.createElement('button');
        retryBtn.className = 'btn btn--brett btn-sm';
