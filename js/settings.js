...
(function () {
  ...
  $('update-app').addEventListener('click', function () {
    ...
    // Nur Cache leeren – SW bleibt registriert, damit iOS im Standalone-Modus bleibt.
    // location.replace statt reload() verhindert das Herausspringen aus der PWA.
    // localStorage (Spaziergänge, Schätze, Erfolge) bleibt vollständig erhalten.
    var p = window.caches
    ...
    p.then(function () {
      st.hidden = false;
      st.textContent = 'Cache geleert – App wird neu geladen …';
