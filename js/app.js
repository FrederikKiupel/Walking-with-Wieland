...
(function (global) {
  ...
  var DEFAULT_ORS_KEY = 'eyJvcmciOiI1YjNjZTM1OTc4NTExMTAwMDFjZjYyNDgiLCJpZCI6IjFmYjMzNzRlNDhlNDM0MDczMTFjZDBlNTMzMjRlZWM5OTAzOTA4YWI5MjBiNzY0YTExNjcxYjNkIiwiaCI6Im11cm11cjY0In0=';
  ...
  function defaultState() {
    return {
      settings: { home: null, orsKey: DEFAULT_ORS_KEY, weeklyGoal: 4, testMode: false, debugExMode: false },
  ...
  function load() {
    ...
    try {
      ...
      if (raw) {
        ...
        if (!s.settings.orsKey) s.settings.orsKey = DEFAULT_ORS_KEY;
  ...
  // "Etwas anderes, Wieland" — neue Übung aus derselben Welt
  ...
  function navHTML(active) {
    ...
    return '<nav class="nav" aria-label="Hauptnavigation">' + items.map(function (it) {
      ...
      return '<a class="nav-item' + (on ? ' is-active' : '') + '" href="' + it.href + '"' +
        ...
        ' onclick="event.preventDefault();event.stopPropagation();location.replace(\'' + it.href + '\')">' +
  ...
  /* ---------- iOS PWA: History-Chrome unterdrücken ----------
     iOS 17+ zeigt bei <a href>-Navigation eine Zurück-Leiste (X + URL).
     location.replace() ersetzt den History-Eintrag statt einen neuen
     anzulegen – iOS zeigt dann kein Browser-Chrome mehr. */
  document.addEventListener('click', function (e) {
    var a = e.target.closest('a[href]');
    if (!a) return;
    var href = a.getAttribute('href');
    // Nur relative interne Links abfangen (keine http//, keine #Anker, keine mailto:)
    if (href && href.indexOf('//') === -1 && href.indexOf(':') === -1 && href.charAt(0) !== '#') {
      e.preventDefault();
